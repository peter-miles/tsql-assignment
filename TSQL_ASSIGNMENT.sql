...
-- EXEC GET_CUSTOMER_STRING @PCUSTID = 1, @PRETURNSTRING = @CUSTOUTPUT OUTPUT;
-- BEGIN
--     PRINT @CUSTOUTPUT;
-- END;
-- DECLARE @PRODOUTPUT NVARCHAR(1000);
-- EXEC GET_PROD_STRING @PPRODID = 1001, @PRETURNSTRING = @PRODOUTPUT OUTPUT;
-- BEGIN
--     PRINT @PRODOUTPUT;
-- END;
...
-- For GET_ALL_CUSTOMERS I wasn't able to figure out how to use SYS_REFCURSOR in TSQL and after some
-- research I came to the conclusion that SYS_REFCURSOR is only in Oracle databases.
-- I ednded up doing my own solution which I found here: https://www.mssqltips.com/sqlservertip/6308/different-ways-to-write-a-cursor-in-sql-server/
CREATE PROCEDURE GET_ALL_CUSTOMERS @POUTCUR CURSOR VARYING OUTPUT AS
BEGIN
    BEGIN TRY
        DECLARE @OCUSTID INT, @OCUSTNAME NVARCHAR(100), @OSTATUS NVARCHAR(7), @OSALES_YTD MONEY;
        SET @POUTCUR = CURSOR FOR SELECT CUSTID, CUSTNAME, [STATUS], SALES_YTD FROM CUSTOMER;
        OPEN @POUTCUR;
    END TRY
    BEGIN CATCH
        BEGIN
...
-- FETCH NEXT FROM @POUTCUR INTO @OCUSTID, @OCUSTNAME, @OSTATUS, @OSALES_YTD;
-- WHILE @@FETCH_STATUS = 0
-- BEGIN
--     PRINT CONCAT('Custid: ', @OCUSTID, ' Name: ', @OCUSTNAME, ' Status: ', @OSTATUS, ' Sales YTD: ', @OSALES_YTD);
--     FETCH NEXT FROM @POUTCUR INTO @OCUSTID, @OCUSTNAME, @OSTATUS, @OSALES_YTD;
-- END;
-- CLOSE @POUTCUR;
-- DEALLOCATE @POUTCUR;
...
-- DECLARE @POUTCUR CURSOR;
-- DECLARE @OPRODID INT, @OPRODNAME NVARCHAR(100), @SELLING_PRICE MONEY, @OSALES_YTD MONEY;
-- EXEC GET_ALL_PRODUCTS @POUTCUR = @POUTCUR OUTPUT;
-- FETCH NEXT FROM @POUTCUR INTO @OPRODID, @OPRODNAME, @SELLING_PRICE, @OSALES_YTD;
-- WHILE @@FETCH_STATUS = 0
-- BEGIN
--     PRINT CONCAT('Prodid: ', @OPRODID, ' Name: ', @OPRODNAME, ' Selling price: ', @SELLING_PRICE, ' Sales YTD: ', @OSALES_YTD);
--     FETCH NEXT FROM @POUTCUR INTO @OPRODID, @OPRODNAME, @SELLING_PRICE, @OSALES_YTD;
-- END;
-- CLOSE @POUTCUR;
-- DEALLOCATE @POUTCUR;
...
-- DECLARE @POUTCUR CURSOR;
-- DECLARE @OSALEID BIGINT, @OCUSTID INT, @OPRODID INT, @OQTY INT, @OPRICE MONEY, @OSALEDATE DATE;
-- EXEC GET_ALL_SALES @POUTCUR = @POUTCUR OUTPUT;
-- FETCH NEXT FROM @POUTCUR INTO @OSALEID, @OCUSTID, @OPRODID, @OQTY, @OPRICE, @OSALEDATE;
-- WHILE @@FETCH_STATUS = 0
-- BEGIN
--     PRINT CONCAT('Saleid: ', @OSALEID, ' Custid: ', @OCUSTID, ' Prodid: ', @OPRODID, ' Qty: ', @OQTY, ' Price: ', @OPRICE, ' Sale date: ', @OSALEDATE);
--     FETCH NEXT FROM @POUTCUR INTO @OSALEID, @OCUSTID, @OPRODID, @OQTY, @OPRICE, @OSALEDATE;
-- END;
-- CLOSE @POUTCUR;
-- DEALLOCATE @POUTCUR;
...
BEGIN
    BEGIN TRY
        -- Because SALE_SEQ doesn't define a starting value it defaults to min BIGINT value causing error
        -- I'm not sure whether I'm supposed to change the DDL so I haven't adjusted the SALE_SEQ
        DECLARE @MINSALEID BIGINT;
        SELECT @MINSALEID = MIN(SALEID) FROM SALE;

        IF @MINSALEID != NULL -- Will always be NULL because overflow in MIN() function
...
-- EXEC DELETE_CUSTOMER @PCUSTID = 4;
-- SELECT * FROM CUSTOMER;
